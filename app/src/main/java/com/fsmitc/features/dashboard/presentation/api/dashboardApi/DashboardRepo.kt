package com.fsmitc.features.dashboard.presentation.api.dashboardApi
...
import com.fsmitc.features.addAttendence.model.AddAttendenceImageInput
import com.fsmitc.features.dashboard.presentation.DashboardActivity
import com.fsmitc.features.dashboard.presentation.model.DayStartEndImageInput
import com.fsmitc.features.location.LocationFuzedService
import com.fsmitc.features.login.model.AlarmSelfieInput
...
class DashboardRepo(val apiService: DashboardApi) {
    ...
    fun dayStartWithImage(image: String, context: Context): Observable<BaseResponse> {
        ...
        if (profile_img_file != null && profile_img_file.exists()) {
            val profileImgBody = RequestBody.create(MediaType.parse("multipart/form-data"), profile_img_file)
            profile_img_data = MultipartBody.Part.createFormData("image", profile_img_file.name.dropLast(41).drop(18)+".jpg", profileImgBody)
        } else {
            var mFile: File? = null
